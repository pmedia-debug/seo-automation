...
def auth_callback():
    ...
    # Allow HTTP redirect in local dev
    auth_resp = request.url
    if request.url.startswith("http://") and os.environ.get("OAUTHLIB_INSECURE_TRANSPORT"):
        auth_resp = auth_resp  # already http, fine
...
def api_generate():
    ...
    schema_type = body.get("schema_type", "product")

    if not docs_urls:
        return jsonify({"error": "No docs_urls provided."}), 400
    if schema_type not in ("product", "blog"):
        return jsonify({"error": "schema_type must be 'product' or 'blog'."}), 400
...
<header>
  ...
  <nav>
    <a id="auth-pill" class="disconnected" href="#">&#8230;</a>
  </nav>
</header>

<div class="hero">
  <h1>SEO Schemas from<br/>Google Docs, Instantly</h1>
  <p>Paste your doc links, choose a schema type &mdash; get Breadcrumb, FAQ, Product or Blog schemas ready to copy.</p>
</div>
...
<div class="container">
  ...
  <div class="card">
    ...
    <label class="lbl">Schema Type</label>
    <div class="toggle-group">
      <input type="radio" id="t-product" name="stype" value="product" class="toggle-opt" checked/>
      ...
      <label for="t-blog" class="toggle-lbl"><span class="icon">&#128221;</span>Blog Schema</label>
    </div>
  ...
  try {
    ...
    data.results.forEach((pg, idx) => {
      ...
      if (pg.error) {
        ...
        if (pg.banner_url) html += chip('&#127968; Banner', 'Auto-detected');
        ...
        html += '</div>';
        html += codeBlock('Breadcrumb Schema', pg.breadcrumb_schema);
        if (pg.faq_schema)     html += codeBlock('FAQ Schema', pg.faq_schema);
        if (pg.product_schema) html += codeBlock('Product Schema', pg.product_schema);
        if (pg.blog_schema)    html += codeBlock('Blog / Article Schema', pg.blog_schema);
        html += '</div>';
      }
